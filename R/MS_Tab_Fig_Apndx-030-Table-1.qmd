...
hrs16_cog_025_n2993 <- hrs16_cog_025 |> 
  dplyr::inner_join(n2993, by = c("HHID", "PN"))
...
labeled_list <- list(
  ...
  map(label_categorical_vars)

foo <- n2993 %>%
  mutate(n2993=1)

goo <- hrs16_cog_025 %>%
  left_join(foo, by = c("HHID", "PN")) %>%
  mutate(n2993 = case_when(is.na(n2993) ~ 0,
                           !is.na(n2993) ~ n2993))

goo %>%
  select(n2993, female, rage, RACE, rHISP, SCHLYRS) %>%
  mutate(female = forcats::fct_relevel(female, "Female", "Male"),
         RACE = forcats::fct_relevel(RACE, "White", "Black", "All other groups", "Not collected"),
         rHISP = forcats::fct_relevel(rHISP, "Not Hispanic", "Hispanic", "Not obtained")
         ) %>%
  gtsummary::tbl_summary(by = n2993,
                         statistic = gtsummary::all_continuous() ~ c("{mean} ({sd})"),
                         missing_text = "Blank, partial interview",
                         digits = gtsummary::all_continuous() ~ 1,
                         label = list(female = "Sex",
                                      rage = "Age (in years)",
                                      RACE = "Race",
                                      rHISP = "Hispanic origin",
                                      SCHLYRS = "Years of education")
                         ) %>%
  gtsummary::add_overall() %>%
  gtsummary::modify_column_hide("stat_1") %>%
  gtsummary::modify_header(stat_0 = "**HRS 2016 Wave** \n N = {gtsummary::style_number(n)}",
                           stat_2 = "**HCAP subsample** \nN = {gtsummary::style_number(n)}") 
...
summary_all <- purrr::map_df(vars, summarize_variable, data = hrs16_cog_025)
...
summary_n2993 <- purrr::map_df(vars, summarize_variable, data = hrs16_cog_025_n2993)
